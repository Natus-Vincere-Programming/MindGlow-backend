...
import com.natusvincere.mindglow.user.response.*;
...
public class UserService {
    ...
    public TeachersResponse getTeachers(boolean enabled) {
        return TeachersResponse.builder()
            .users(mapUsersToResponse(userRepository.findAllByRoleAndEnabled(Role.TEACHER, enabled).stream()))
                .build();
    }

    public TeachersResponse getTeachers(String startWith, boolean enabled) {
        Set<UserResponse> users = new HashSet<>();
        users.addAll(mapUsersToResponse(userRepository.findAllByFirstnameStartingWithAndRoleAndEnabled(startWith, Role.TEACHER, enabled).stream()));
        users.addAll(mapUsersToResponse(userRepository.findAllByLastnameStartingWithAndRoleAndEnabled(startWith, Role.TEACHER, enabled).stream()));
        users.addAll(mapUsersToResponse(userRepository.findAllByEmailStartingWithAndRoleAndEnabled(startWith, Role.TEACHER, enabled).stream()));
        return TeachersResponse.builder()
                .users(users)
                .build();
    }

    public TeachersResponse getTeachers(int page, int size, String startWith, boolean enabled) {
        boolean hasNext = false;
        PageRequest request = PageRequest.of(page, size);
        Set<UserResponse> users = new HashSet<>();
        Slice<User> allByFirstnameStartingWithAndRole = userRepository.findAllByFirstnameStartingWithAndRoleAndEnabled(startWith, Role.TEACHER, request, enabled);
        Slice<User> allByLastnameStartingWithAndRole = userRepository.findAllByLastnameStartingWithAndRoleAndEnabled(startWith, Role.TEACHER, request, enabled);
        Slice<User> allByEmailStartingWithAndRole = userRepository.findAllByEmailStartingWithAndRoleAndEnabled(startWith, Role.TEACHER, request, enabled);
        if (allByFirstnameStartingWithAndRole.hasNext() || allByLastnameStartingWithAndRole.hasNext() || allByEmailStartingWithAndRole.hasNext()) {
            hasNext = true;
        }
        users.addAll(mapUsersToResponse(allByFirstnameStartingWithAndRole.getContent().stream()));
    ...
    public TeachersResponse getTeachers(int page, int size, boolean enabled) {
        PageRequest request = PageRequest.of(page, size);
        Slice<User> allByRole = userRepository.findAllByRoleAndEnabled(Role.TEACHER, request, enabled);
        return TeachersResponse.builder()
                .users(mapUsersToResponse(allByRole.getContent().stream()))
                .hasNext(allByRole.hasNext())
                .build();
